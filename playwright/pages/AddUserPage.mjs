...
class AddUserPage extends UtilityMethods {
  ...
  async Adduser() {
    ...
    if (parseInt(divValue.charAt(7), 10) === 0) {
      ...
      await this.locateFill(
        "#FirstName",
        process.env.PLAYWRIGHT_ADDUSERFIRSTNAME
      );
      ...
      await this.locateFill(
        "#RegistrationNumber",
        process.env.PLAYWRIGHT_RegistrationNumber
      );
      await this.click("#Designation");
      await this.locateFill(
        "#Designation",
        process.env.PLAYWRIGHT_ADDUSERDESIGNATION
      );
      await this.click("#Designation");
      await this.locateFill(
        "#Designation",
      ...
      await this.locateFill(
        "#Department",
        process.env.PLAYWRIGHT_ADDUSERDEPARTMENT
      );
      await this.click("#PrimaryPlaceOfWork");
      await this.locateFill(
        "#PrimaryPlaceOfWork",
        process.env.PLAYWRIGHT_ADDUSERPLACEOFWORK
      );
      await this.click("#LineManagerName");
      await this.locateFill(
        "#LineManagerName",
        process.env.PLAYWRIGHT_ADDUSERLINEMANAGERNAME
      );
      await this.click("#LineManagerSurname");
      await this.locateFill(
        "#LineManagerSurname",
        process.env.PLAYWRIGHT_ADDUSERLINEMANAGERSURNAME
      );
      await this.click("#passwordInput");
      await this.locateFill(
        "#passwordInput",
        process.env.PLAYWRIGHT_ADDUSERPASSWORD
      );
      await this.SelectCheckbox(
        "label",
        process.env.PLAYWRIGHT_ADDUSERCHK1,
        "span"
      );
      await this.SelectCheckbox(
        "label",
        process.env.PLAYWRIGHT_ADDUSERCHK2,
        "span"
      );
      await this.SelectCheckbox(
        "label",
        process.env.PLAYWRIGHT_ADDUSERCHK3,
        "span"
      );
      await this.SelectCheckbox(
        "label",
        process.env.PLAYWRIGHT_ADDUSERCHK4,
        "span"
      );
      await this.SelectCheckbox(
        "label",
        process.env.PLAYWRIGHT_ADDUSERCHK5,
        "span"
      );
      await this.SelectCheckbox(
        "label",
        process.env.PLAYWRIGHT_ADDUSERCHK6,
        "span"
      );
      await this.SelectCheckbox(
        "label",
        process.env.PLAYWRIGHT_ADDUSERCHK7,
        "span"
      );
      await this.SelectCheckbox(
        "label",
        process.env.PLAYWRIGHT_ADDUSERCHK8,
        "span"
      );
      await this.SelectCheckbox(
        "label",
        process.env.PLAYWRIGHT_ADDUSERCHK9,
        "span"
      );
      await this.page
        .getByText(
          "Account Type Local Domain AzureAD UserName First Name Surname Email Address"
        )
        .click();
