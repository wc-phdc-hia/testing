...
class LabsPage extends UtilityMethods {
    ...
    async gotoLabsPage() {
        ...
        await this.page.waitForTimeout(3000);
        await this.page.getByRole('treeitem', { name: process.env.PLAYWRIGHT_LABSCODE1 }).getByRole('checkbox').click();
        ...
        await this.page.getByRole('treeitem', { name: process.env.PLAYWRIGHT_LABSCODE2 }).getByRole('checkbox').click();
        await this.clickButtonByRoleAndName('link', 'Filter');
        await this.click('#pivot-off');
        await this.page.waitForTimeout(3000);
        await this.clickButtonByRoleAndName('button', 'BY DATE');
        await this.click('#pivot-off');
        await this.takeScreenshotAndAttach('Perform Action');
    }
}
