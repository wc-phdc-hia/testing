...
class MedicinesPage extends UtilityMethods 
{
 ...
 await this.page.getByRole('treeitem', { name: process.env.PLAYWRIGHT_MEDICINECODE1 }).getByRole('checkbox').click();
 await this.clickButtonByRoleAndName('link', 'Filter');
 await this.click('#filter-off');
 await this.click('#pivot-drug-on');
 await this.page.getByRole('treeitem', { name: process.env.PLAYWRIGHT_MEDICINECODE2 }).getByRole('checkbox').click();
 await this.clickButtonByRoleAndName('link', 'Filter');
     }
}
